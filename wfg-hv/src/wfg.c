...
OBJECTIVE get_ref_point_def_value(int m_objs, int verbose_flag) {
    // Return the default value for the reference point for hypervolume computation according
    // to [Ishibuchi18] How to Specify a Reference Point in Hypervolume Calculation for Fair Performance Comparison
    ...
    switch (m_objs) {
        ...
        case 8:  { h =  3 ; break; }
        case 10: { h =  3 ; break; }
        default: { printf("There is not a default value for H for m_objs=%d", m_objs); exit(0); }
    }
    
    def_value = 1 + (1/h);
    
    if (verbose_flag)
        printf("Using H=%.2f for m_objs=%d (r = 1 + 1/%.2f = %.4f)\n", h, m_objs, h, def_value);
...
// processes each front from the file 
{
    ...
    ind_filename = arg_index;
    ind_ref_vect = arg_index+1;
    
    // debug
    if (verbose_flag) {
        
        printf ("verbose flag: %d, arg_index: %d, argc: %d, ind_filename: %d, ind_ref_vect: %d\n",
                verbose_flag, arg_index, argc, ind_filename, ind_ref_vect);
	...
	FILECONTENTS *f = readFile(argv[ind_filename]);

	// find the biggest fronts
	maxm = 0;           // number of points
	maxn = 0;           // number of objectives
    ...
    // added to handle an empty pop
    if (maxn == 0 || maxm == 0) {
        
        if (verbose_flag)
            printf ("it is an empty pop, the hv value is zero\n");
    ...
    OBJECTIVE total_volume = 1;                                         // added for normalizing hv
	ref.objectives = malloc(sizeof(OBJECTIVE) * maxn);
	if (argc == ind_ref_vect) {
		printf("No reference point provided: exiting\n");
        return 0;
		//for (int i = 0; i < maxn; i++) {
		//	ref.objectives[i] = 0;
		//}
	}
	else if (argc - ind_ref_vect != maxn) {
		printf("Your reference point should have %d values: exiting\n", maxn);
		return 0;
	}
	else {
		for (int i = ind_ref_vect; i < argc; i++) {
            
            int pos = i - ind_ref_vect;
			ref.objectives[pos] = atof(argv[i]);
            
            // added to use def_value
            // r = 1+1/H as value for the reference vector
            if (ref.objectives[pos] < 1)
                ref.objectives[pos] = get_ref_point_def_value(maxn, verbose_flag);
                
            // added for normalizing hv as recommended by [Cheng16]
            total_volume *= ref.objectives[pos];
		}
	}
    ...
    if (verbose_flag) {
        ...
        for (int counter=0; counter<maxn; counter++)
            printf("%d, %.2f \n", counter, ref.objectives[counter]);
    }
	...
	for (int i = 0; i < f->nFronts; i++) {      
        ...
        double hv_raw  = hv(f->fronts[i]);
        double hv_norm = hv_raw / total_volume;
